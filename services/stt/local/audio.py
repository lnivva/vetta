...
class AudioResolver:
    ...
    def _fetch_uri(self, uri: str) -> bytes:
        """
        Download audio from a remote URI with streaming size checks.

        Raises:
            AudioValidationError: If the download exceeds the size limit.
            AudioFetchError: On any network or HTTP error.
        """
        try:
            response = requests.get(uri, timeout=15, stream=True)
